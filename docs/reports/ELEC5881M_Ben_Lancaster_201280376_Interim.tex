...
\section{Parallelism and Amdahl's Law}
...
In serial software programs there may exist many potential opportunities for parallelism. 

\textit{Speedup} is a term used to describe the potential time improvements to perform an algorithm.
...
This project will utilise a loosely coupled architecture due to it's easier scalability implementation and my previous experience with single-core processors. Although it will require a scheduler to access the MTS, the experience and knowledge gained from this task will be greatly beneficial for future projects.
...
The "critical path" in an algorithm is described as the longest sequence of instructions that must be performed sequentially due to data dependencies within the instruction sequence  \cite{bohme2014characterizing}.
...
function has_dependency(a, b):
    ...
    y = x + b
\end{minted}
\caption{Sequence of instructions where a data dependency exists between instructions 2 and 3 -- Instruction 3 must wait for variable x to be ready.}
\label{fig:}
\end{figure}
\end{minipage}
\begin{minipage}{0.05\textwidth}\hfill\end{minipage}
...
function no_dependency(a, b):
    ...
    y = b + 2
\end{minted}
\caption{Sequence of instructions with no data dependencies. This sequence can be ran in parallel using multiple processing cores.}
\label{fig:}
\end{figure}
\end{minipage}

\subsection{Access to shared memory}
...
\item{\textbf{Analyse performance of serial and parallel software algorithms, such as parallel DFT \cite{dft}, on the processor.}\\
...
Now that this time period is closer, time allocations have been assigned for stages 4, 7, and 8.
...
\begin{table}[h]
	...
	\\ \hline
    3.4 & Cache memory design \& impl & \st{Apr 22} &  & Not planned
    \\ \specialrule{2pt}{-2pt}{0pt}
...
\subsection{Updated Project Gantt Chart}
\begin{figure}[H]
\centering 
